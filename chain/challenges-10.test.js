...
const count = (target, input) => {
  input.reduce( (ans, arr) => ans +arr.reduce((innerAns, num) => innerAns + (num === target ?1 : 0), 0), 0); //Michelle's answer from code review
};
...
//     value.filter(num => typeof(num) === 'number' && !(num % 5))
//     .map(num => **num)
//   })
// };
